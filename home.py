...
banco_questoes = sqlite3.connect("banco_questoes1.db", check_same_thread=False)
cursor_questoes = banco_questoes.cursor()
cursor = banco.cursor()
cursor.execute(
    "CREATE TABLE IF NOT EXISTS usuarios5 (usuario text,senha text,level integer,data text,consecutivos integer,exp integer,icone text,pontos_nivel int)"
)


def main(page: Page):
    opcao_nivel = ft.Text("0")
    botao_confirmar_icone = ft.ElevatedButton(
        content=ft.Text("Confirmar", size=20),
        width=200,
        height=50,
        color="black",
        bgcolor="white",
    ...
    icone_usuario_jogando = ft.Image(
        "./assets/carangueijo_sem_fundo.png", width=400, height=130
    )
    exp_value = ft.Text("0")
    ...
    texto_pergunta = ft.Text(
        "Questão 1/6", size=50, color="black", weight=ft.FontWeight.BOLD
    )
    texto_a_ser_falado = ft.Text(
        " ",
        size=25,
        color="white",
    ...
    botao_alternativa_A = ft.ElevatedButton(
    ...
    botao_alternativa_B = ft.ElevatedButton(
        content=ft.Text("", size=20),
        width=500,
        height=50,
        color="black",
    ...
    botao_alternativa_C = ft.ElevatedButton(
        content=ft.Text("", size=20),
        width=500,
        height=50,
        color="black",
    ...
    botao_alternativa_D = ft.ElevatedButton(
        content=ft.Text("", size=20),
        width=500,
        height=50,
        color="black",
    ...
    botao_confirmar_alternativa = ft.ElevatedButton(
        content=ft.Text("CONFIRMAR", size=20),
        width=200,
        height=50,
        color="black",
    ...
    def ir_para_proxima_questao(e):
        if opcao_nivel.value == "0":
            consulta = cursor_questoes.execute(
                "SELECT * FROM questoesfaceis WHERE frances=?", (texto_a_ser_falado.value,)
            )
        ...
        elif opcao_nivel.value == "1":
            consulta = cursor_questoes.execute(
                "SELECT * FROM questoesmedias WHERE frances=?", (texto_a_ser_falado.value,)
            )
            res = consulta.fetchall()
            botao_alternativa_A.content = ft.Text(res[0][1])
            botao_alternativa_B.content = ft.Text(res[0][2])
            ...
            if texto_pergunta.value != "Questão 6/6":
                texto_pergunta.value = f"Questão {int(texto_pergunta.value[8])+1}/6"
            page.go("/jogando_alternativa")
        elif opcao_nivel.value == "2":
            consulta = cursor_questoes.execute(
                "SELECT * FROM questoesdificeis WHERE frances=?", (texto_a_ser_falado.value,)
            )
            res = consulta.fetchall()
            botao_alternativa_A.content = ft.Text(res[0][1])
            botao_alternativa_B.content = ft.Text(res[0][2])
    ...
    def fechar_campo(e):
        ...
        container_botao_falar.height = 60
        if opcao_nivel.value == "0":
            texto = random.randint(0, 386)
        elif opcao_nivel.value == "1":
            texto = random.randint(0, 373)
        elif opcao_nivel.value == "2":
            texto = random.randint(0, 349)
        texto_falado_indice.value = str(texto)
        if opcao_nivel.value == "0":  
            consulta = cursor_questoes.execute(
            "SELECT * FROM questoesfaceis"
        )
        elif opcao_nivel.value == "1":
            consulta = cursor_questoes.execute(
            "SELECT * FROM questoesmedias"
        )
        elif opcao_nivel.value == "2":
            consulta = cursor_questoes.execute(
            "SELECT * FROM questoesdificeis" 
        )
        res = consulta.fetchall()
        texto_questao = res[texto][0]
        texto_a_ser_falado.value = texto_questao
        text_consecutivos.value = f"Você está jogando a {res[0][4]} dias consecutivos"
        page.update()
        page.go("/jogando")
        page.update()
    ...
    def finalizou_jogo(e):
        consulta = cursor.execute(
            "SELECT * FROM usuarios5 WHERE usuario = ?", (text_nome_usuario.value,)
        )
        res = consulta.fetchall()
        ...
        if int(exp_value.value) >= int(res[0][7]):
            ...
            cursor.execute(
                "UPDATE usuarios5 SET pontos_nivel = ? WHERE usuario = ?",
                (
                    int(res[0][7]) + (int(text_level.value[6:]) - 1) * 10,
                    text_nome_usuario.value,
                ),
            )
        data = str(res[0][3])
        string = data.split("-")
        ...
        mes = int(string[1])
        dia = int(string[2])
        print(datetime.date(ano,mes,dia))
        if datetime.date.today() - datetime.timedelta(days=1) == datetime.date(ano,mes,dia):
                    sql = ''' UPDATE usuarios5
                SET consecutivos = ?
                WHERE usuario = ?'''
                    consecutivos = int(res[0][4])
                    text_consecutivos.value = f"Você está jogando a {consecutivos + 1} dias consecutivos"
                    print(consecutivos)
        ...
        cursor.execute(
            "UPDATE usuarios5 SET exp = ? WHERE usuario = ?",
            (exp_value.value, text_nome_usuario.value),
        )
        banco.commit()
        cursor.execute(
            "UPDATE usuarios5 SET level = ? WHERE usuario = ?",
            (text_level.value[6], text_nome_usuario.value),
        )
        page.go("/jogar")
        banco.commit()
        ...
        if texto_pergunta.value != "Questão 6/6":
            ...
            if int(text_valor_clickado.value) == res[0][5]:
                ...
                errou_questão.actions = [
                    ft.TextButton("Próxima questão", on_click=fechar_campo)
                ]
                errou_questão.open = True
                if opcao_nivel.value == "0":  
                    texto_pontuacao_soma.value = str(int(texto_pontuacao_soma.value) + 3)
                elif opcao_nivel.value == "1":
                    texto_pontuacao_soma.value = str(int(texto_pontuacao_soma.value) + 4)
                elif opcao_nivel.value == "2":
                    texto_pontuacao_soma.value = str(int(texto_pontuacao_soma.value) + 6)
                page.update()
            else:
                page.dialog = errou_questão
            ...
            page.dialog = errou_questão
            errou_questão.open = True
            errou_questão.title = ft.Row(
                [
                    ft.Text(f"Parabéns você ganhou {texto_pontuacao_soma.value}"),
                    ft.Icon(ft.icons.STAR_PURPLE500, color="yellow"),
                    ft.Text("!!"),
                ]
            )
            errou_questão.content = ft.Text("")
            errou_questão.actions = [ft.TextButton("Sair", on_click=finalizou_jogo)]
        botao_alternativa_A.bgcolor = "white"
        botao_alternativa_B.bgcolor = "white"
        botao_alternativa_C.bgcolor = "white"
        botao_alternativa_D.bgcolor = "white"
    ...
    botao_icone_triunfo = ft.ElevatedButton(
        "", opacity=0, on_click=triunfo, height=100, width=100
    )
    icone_arco_triunfo = ft.Container(
        ft.Stack(
            [ft.Image("./assets/arco-do-triunfo.png", width=100), botao_icone_triunfo]
        ),
        border_radius=30,
    )
    ...
    botao_bandeira_brasil = ft.ElevatedButton(
        "", opacity=0, on_click=bandeira_brasil, height=100, width=100
    )
    icone_bandeira_brasil = ft.Container(
        ft.Stack(
            [
                ft.Container(
                    ft.Image("./assets/bandeira_brasil.png", width=100, height=50),
    ...
    botao_bandeira_frança = ft.ElevatedButton(
        "", opacity=0, on_click=bandeira_frança, height=100, width=100
    )
    icone_bandeira_frança = ft.Container(
        ft.Stack(
            [
                ft.Container(
                    ft.Image("./assets/bandeira_frança.png", width=100),
    ...
    botao_bola_francesa = ft.ElevatedButton(
        "", opacity=0, on_click=bola_francesa, height=100, width=100
    )
    icone_bola_francesa = ft.Container(
        ft.Stack(
            [
                ft.Container(
                    ft.Image("./assets/bola_francesa.png", width=100),
    ...
    botao_padrao = ft.ElevatedButton(
        "", opacity=0, on_click=padrao, height=100, width=100
    )
    icone_padrao = ft.Container(
        ft.Stack(
            [ft.Image("./assets/carangueijo_sem_fundo.png", width=100), botao_padrao]
        ),
        border_radius=30,
    )
    ...
    botao_caranguejo_estressado = ft.ElevatedButton(
        "", opacity=0, on_click=caranguejo_estressado, height=100, width=100
    )
    icone_caranguejo_estressado = ft.Container(
        ft.Stack(
            [
                ft.Container(
                    ft.Image("./assets/caranguejo-puto.png", width=100),
                    margin=ft.margin.only(top=15),
                ),
    ...
    botao_coelho = ft.ElevatedButton(
        "", opacity=0, on_click=coelho, height=100, width=100
    )
    icone_coelho = ft.Container(
        ft.Stack(
            [
                ft.Container(
                    ft.Image("./assets/coelho fofinho.png", width=100),
                    margin=ft.margin.only(top=15),
                ),
                botao_coelho,
            ]
    ...
    botao_dinossauro_verde = ft.ElevatedButton(
        "", opacity=0, on_click=dinossauro_verde, height=100, width=100
    )
    icone_dinossauro_verde = ft.Container(
        ft.Stack(
            [
    ...
    botao_dinossauro_laranja = ft.ElevatedButton(
        "", opacity=0, on_click=dinossauro_laranja, height=100, width=100
    )
    icone_dinossauro_laranja = ft.Container(
        ft.Stack(
            [
    ...
    icone_leao = ft.Container(
        ft.Stack([ft.Image("./assets/leão-pixelart.png", width=100), botao_leao]),
        border_radius=30,
    )
    ...
    icone_maça = ft.Container(
        ft.Stack([ft.Image("./assets/maça.png", width=100), botao_maça]),
        border_radius=30,
    )
    ...
    botao_mimico = ft.ElevatedButton(
        "", opacity=0, on_click=mimico, height=100, width=100
    )
    icone_mimico = ft.Container(
        ft.Stack([ft.Image("./assets/mimico.png", width=100), botao_mimico]),
        border_radius=30,
    )
    ...
    botao_napoleao = ft.ElevatedButton(
        "", opacity=0, on_click=napoleao, height=100, width=100
    )
    icone_napoleao = ft.Container(
        ft.Stack([ft.Image("./assets/napoleao.png", width=100), botao_napoleao]),
        border_radius=30,
    )
    ...
    botao_neymar = ft.ElevatedButton(
        "", opacity=0, on_click=neymar, height=100, width=100
    )
    icone_neymar = ft.Container(
        ft.Stack([ft.Image("./assets/neymar.png", width=100), botao_neymar]),
        border_radius=30,
    )
    ...
    botao_pizza = ft.ElevatedButton(
        "", opacity=0, on_click=pizza, height=100, width=100
    )
    icone_pizza = ft.Container(
        ft.Stack(
            [
                ft.Container(
                    ft.Image("./assets/pizza.png", width=100),
                    margin=ft.margin.only(top=15),
                ),
                botao_pizza,
            ]
    ...
    botao_tubarao_azul = ft.ElevatedButton(
        "", opacity=0, on_click=tubarao_azul, height=100, width=100
    )
    icone_tubarao_azul = ft.Container(
        ft.Stack(
            [
                ft.Container(
                    ft.Image("./assets/tubarao_azul.png", width=100),
                    margin=ft.margin.only(top=15),
                ),
                botao_tubarao_azul,
            ]
    ...
    botao_tubarao_cinza = ft.ElevatedButton(
        "", opacity=0, on_click=tubarao_cinza, height=100, width=100
    )
    icone_tubarao_cinza = ft.Container(
        ft.Stack(
            [ft.Image("./assets/tubarao_cinza.png", width=100), botao_tubarao_cinza]
        ),
        border_radius=30,
    )
    ...
    def confimar_icone(e):
        cursor.execute(
            "UPDATE usuarios5 SET icone = ? WHERE usuario = ?",
            (icone_usuario.src, text_nome_usuario.value),
        )
        page.dialog = confirmou_icone
    ...
    icone_urso = ft.Container(
        ft.Stack([ft.Image("./assets/urso.png", width=100), botao_urso]),
        border_radius=30,
    )
    ...
    text_dias_consecutivos = ft.Text(
        "Você está jogando a 5 dias consecutivos", size=20, color="white"
    )
    botao_clicavel = ft.ElevatedButton("", opacity=0)  # botao em cima da seta de voltar
    botao_voltar = ft.Stack(
        [ft.Image("Vector-removebg-preview.png", width=50), botao_clicavel]
    )  # botao de voltar
    ...
    progressobar = ft.ProgressBar(
        value=0,
        width=250,
        height=30,
        color="#ffd600",
        border_radius=20,
        semantics_label="13/100",
    )
    texto_exp = ft.Text("0/100", size=20)
    texto_estrela = ft.Container(
        ft.Icon(ft.icons.STAR_PURPLE500, color="yellow"),
        alignment=ft.alignment.center,
        margin=ft.margin.all(0),
    )
    texto_final_exp = ft.Container(
        ft.Row([texto_exp, texto_estrela], alignment=ft.MainAxisAlignment.CENTER),
        alignment=ft.alignment.center,
        padding=0,
    )
    progresso_bar = ft.Container(
        ft.Stack(
            [
                ...
                ft.Container(
                    texto_final_exp, width=250, alignment=ft.alignment.center_right
                ),
            ]
        ),
        alignment=ft.alignment.center,
    ...
    text_consecutivos = ft.Text(
        "0", size=30
    )  # caixa de texto para colocar quantidade de dias consecutivos
    text_mudar_usuario: ft.TextField = ft.TextField(
        label="Novo usuário",
        text_align=ft.TextAlign.LEFT,
        width=240,
        label_style=ft.TextStyle(color="#E77A52"),
        max_length=15,
    )# caixa de seleção para mudar o nome de usuário
    text_confirmar_mudar_usuario: ft.TextField = ft.TextField(
    ...
    text_senha_atual: ft.TextField = ft.TextField(
        label="Senha atual",
        text_align=ft.TextAlign.LEFT,
        width=280,
        password=True,
    ...
    text_confirmar_nova_senha: ft.TextField = ft.TextField(
        label="Nova senha",
        text_align=ft.TextAlign.LEFT,
        width=280,
        password=True,
    ...
    text_nova_senha: ft.TextField = ft.TextField(
    ...
    botao_confirmar_novo_usuario: ft.ElevatedButton = ft.ElevatedButton(
        text="CONFIRMAR",
        width=200,
        color="white",
        bgcolor="#E77A52",
    ...
    botao_confirmar_troca_senha: ft.ElevatedButton = ft.ElevatedButton(
        text="CONFIRMAR",
        width=200,
        color="white",
        bgcolor="#E77A52",
                    ...
                    tabs=[
                        ft.Tab(
                            tab_content=ft.Container(
                                ft.Text("MUDAR NOME DE USUÁRIO", color="black"),
                                width=260,
                                height=50,
                                alignment=ft.alignment.center,
                            ),
                            content=ft.Container(  # Container da caixa de login
                                content=ft.Column(
                                    [
                                        ft.Container(
                                            text_mudar_usuario,
                                        ...
                                        ft.Container(
                                            text_confirmar_mudar_usuario,
                                            alignment=ft.alignment.center,
                                            width=300,
                                        ),
                                        ft.Container(
                                            botao_confirmar_novo_usuario,
                                            alignment=ft.alignment.center,
                                            width=300,
                                        ),
                        ...
                        ft.Tab(
                            tab_content=ft.Container(
                                ft.Text("MUDAR SENHA", color="black"),
                                width=260,
                                height=50,
                                alignment=ft.alignment.center,
                            ),
                            content=ft.Container(
                                ft.Column(
                                    [
                                        ...
                                        ft.Container(
                                            content=text_confirmar_nova_senha,
                                            alignment=ft.alignment.center,
                                        ),
                                        ft.Container(
                                            botao_confirmar_troca_senha,
                                            alignment=ft.alignment.center,
                                        ),
                                    ]
                                ),
                        ...
                        ft.Tab(
                            tab_content=ft.Container(
                                ft.Text("MUDAR ÍCONE DE USUÁRIO", color="black"),
                                width=260,
                                height=50,
                                alignment=ft.alignment.center,
                            ),
                                    ...
                                    [
                                        ft.Row(
                                            [
                                                ...
                                                ft.Container(
                                                    icone_bandeira_brasil,
                                                    alignment=ft.alignment.center,
                                                ),
                                                ft.Container(
                                                    icone_bandeira_frança,
                                                    alignment=ft.alignment.center,
                                                ),
                                                ft.Container(
                                                    icone_bola_francesa,
                                        ...
                                        ft.Row(
                                            [
                                                ...
                                                ft.Container(
                                                    icone_dinossauro_verde,
                                                    alignment=ft.alignment.center,
                                                ),
                                                ft.Container(
                                                    icone_dinossauro_laranja,
                                                    alignment=ft.alignment.center,
                                                ),
                                                ft.Container(
                                                    icone_leao,
    ...
    text_confirmar_senha: ft.TextField = ft.TextField(
    ...
    button_registrar: ft.ElevatedButton = ft.ElevatedButton(
        text="Registrar",
        width=200,
        color="white",
        bgcolor="#E77A52",
    ...
    texto_certo_errado = ft.TextField(
    ...
    def update_usuario(e):  # função para atualizar o nome do usuario no banco
        consulta = cursor.execute(
            "SELECT * FROM usuarios5 WHERE usuario=?", (text_mudar_usuario.value,)
        )
        sql = """ UPDATE usuarios5
                SET usuario = ?
                WHERE usuario = ?"""
    ...
    def registrar_conta(e):  # função para registrar uma conta
        consulta_usuarios = cursor.execute(
            "SELECT usuario FROM usuarios5 WHERE usuario=?", (text_username.value,)
        )
        ...
        if text_username.value != "" and text_password.value != "":
            if len(res) > 0:
                ...
                " " in text_username.value
                or "!" in text_username.value
                or "@" in text_username.value
                or "#" in text_username.value
                or "$" in text_username.value
                or "%" in text_username.value
                ...
                page.dialog = erro_registro
                erro_registro.open = True
                erro_registro.title = ft.Text(
                    "Crie um usuário sem espaço em branco e sem caracteres especiais",
                    color="red",
                )
                page.update()
                ...
                erro_registro.open = True
                erro_registro.title = ft.Text(
                    "Digite uma senha com no mínimo 5 dígitos!!!", color="red"
                )
                page.update()
                ...
                erro_registro.open = True
                erro_registro.title = ft.Text(
                    "As senhas não são iguais!!!", color="red"
                )
                page.update()
            else:
                cursor.execute(
                    "INSERT INTO usuarios5(usuario,senha,level,data,consecutivos,exp,icone,pontos_nivel) VALUES (?,?,?,?,?,?,?,?)",
                    (
                        str(text_username.value),
                        str(text_password.value),
                        1,
                        str(datetime.date.today()),
                        0,
                        0,
                        "./assets/carangueijo_sem_fundo.png",
                        100,
                    ),
                )
                banco.commit()
                texto_erro.value = ""
                texto_acerto.value = (
                    "Usuario cadastrado com sucesso,volte para o início para JOGAR!!"
                )
    ...
    def jogar(e):  # função para entrar na pagina de usuario confirmando o login
        consulta_usuarios = cursor.execute(
            "SELECT * FROM usuarios5 WHERE usuario=?", (text_username.value,)
        )
        res = consulta_usuarios.fetchall()
        contagem = len(res)
        if len(res) > 0:
            ...
            text_dias_consecutivos.value = (
                f"Você está jogando a {res[0][4]} dias consecutivos"
            )
        if len(res) > 0:
            jogador = str(res[0][1])
            ...
            ft.Container(
                ft.Stack(
                    [  # Container com imagem de fundo
                        ft.Image(
                            src="./assets/Home Screan (3).png", fit="cover", height=770
                        ),
                        ft.Container(
    ...
    opcoes_icones = [
        ...
        ft.Icon(ft.icons.MONEY_ROUNDED),
    ]
    ...
    def fechar_configuracoes(e):# função para fechar as configurações
        ...
        res = consulta.fetchall()
        icone = res[0][6]
        icone_usuario.src = str(icone)
        icone_usuario_jogando.src = str(icone)
        icone_urso.bgcolor = "white"
        icone_arco_triunfo.bgcolor = "white"
        icone_bandeira_brasil.bgcolor = "white"
        icone_bandeira_frança.bgcolor = "white"
                                    ...
                                    [
                                        ft.Container(
                                            ft.Container(
                                                ...
                                                height=70,
                                                border_radius=40,
                                                margin=ft.margin.only(right=0, top=10),
                                            ),
                                            alignment=ft.alignment.top_right,
                                        ),
                                        ft.Column(
                    ...
                    [  # Container com imagem de fundo
                        ...
                        ft.Container(
                            ft.Container(  # Container da caixa de login
                                content=ft.Column(
                                    [
                                        ...
                                        ft.Container(
                                            ...
                                            alignment=ft.alignment.center,
                                            width=600,
                                            margin=ft.margin.only(bottom=0, top=-60),
                                        ),
                                        ft.Container(
                                            ft.Column(
                                                [
                                ...
                                width=500,
                                margin=ft.margin.only(
                                    top=(page.window_height - 400) / 2,
                                    bottom=(page.window_height - 200) / 2,
                                ),
                                opacity=1,
                                border=ft.border.all(3, color="#E77A52"),
                                border_radius=10,
                            ...
                            ft.Container(
                                ft.Column(
                                    [
                                        ...
                                        ft.Container(
                                            ft.Text(
                                                "Obs: Quanto maior a dificuldade, mais estrelas você poderá ganhar!!",
                                                color="black",
                                                weight=ft.FontWeight.BOLD,
                                            ),
    ...
    def ir_para_jogo_facil(e):
        ...
        texto = random.randint(0, 386)
        texto_falado_indice.value = str(texto)
        consulta = cursor_questoes.execute("SELECT * FROM questoesfaceis")
        res = consulta.fetchall()
        texto_questao = res[texto][0]
        texto_a_ser_falado.value = texto_questao
        page.update()
        page.go("/jogando")
    def ir_para_jogo_medio(e):
        texto_pergunta.value = "Questão 1/6"
        opcao_nivel.value = "1"
        container_texto_falado.height = 0
        container_contagem.height = 0
        ...
        print(texto)
        texto_falado_indice.value = str(texto)
        consulta = cursor_questoes.execute("SELECT * FROM questoesmedias")
        res = consulta.fetchall()
        texto_questao = res[texto][0]
        texto_a_ser_falado.value = texto_questao
        page.update()
        page.go("/jogando")
    def ir_para_jogo_dificil(e):
        texto_pergunta.value = "Questão 1/6"
        opcao_nivel.value = "2"
        container_texto_falado.height = 0
        container_contagem.height = 0
        ...
        texto = random.randint(0, 349)
        texto_falado_indice.value = str(texto)
        consulta = cursor_questoes.execute("SELECT * FROM questoesdificeis")
        res = consulta.fetchall()
        texto_questao = res[texto][0]
        texto_a_ser_falado.value = texto_questao
        page.update()
    ...
    botao_facil.on_click = ir_para_jogo_facil
    botao_dificil.on_click = ir_para_jogo_dificil
    botao_medio.on_click = ir_para_jogo_medio
    container_texto_falado = ft.Container(
        ft.Row(
            [
                ft.Container(
                    ft.Text(
                        "Fale: ", size=30, color="black", weight=ft.FontWeight.BOLD
                    ),
    ...
    container_contagem = ft.Container(
        texto_contagem, height=0, alignment=ft.alignment.center
    )
    jogando_jogo_falando = ft.View(
        ...
        [
            ft.Stack(
                [
                    ft.Image(
                        src="./assets/Game Screen (write).png", fit="cover", height=770
                    ),
                    ft.Container(
                        ft.Container(
                            ft.Row(
                    ...
                    ft.Container(
                        botao_voltar_no_jogo, margin=ft.margin.only(left=10, top=10)
                    ),
                ],
            )
        ],
    ...
    def route_change(e):
        ...
        if page.route == "/jogar":
            ...
            text_level
            page.update()
            page.views.append(principal_jogo)
        if page.route == "/registrar":
            page.views.append(registrar)
        if page.route == "/dificuldade":
            page.views.append(selecionar_dificuldade)
        progresso.on_change = progressoslider
        if page.route == "/jogando":
            page.update()
            page.views.append(jogando_jogo_falando)
        if page.route == "/jogando_alternativa":
            page.update()
            page.views.append(jogando_jogo_alternativa)
        page.update()
